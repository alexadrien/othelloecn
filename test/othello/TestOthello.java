...
public class TestOthello {
    ...
    public void testGame() {
        ...
        assertTrue(game.getToken(3, 3) == WhiteToken);
        assertTrue(game.getToken(4, 4) == WhiteToken);
        assertTrue(game.getToken(3, 4) == BlackToken);
        assertTrue(game.getToken(4, 3) == BlackToken);

        List<Move> moves = game.possibleMoves(WhitePlayer);
        assertTrue(moves.contains(new Move(4, 2, WhitePlayer)));
        assertTrue(moves.contains(new Move(5, 3, WhitePlayer)));
        assertTrue(moves.contains(new Move(2, 4, WhitePlayer)));
        assertTrue(moves.contains(new Move(3, 5, WhitePlayer)));
        //printMoves(moves);

        game.makeMove(4, 2, WhitePlayer);

        assertTrue(game.getState() == BlackPlayerTurn);
        assertTrue(game.getTokenCount(WhiteToken) == 4);
        assertTrue(game.getTokenCount(BlackToken) == 1);
    ...
    public void testLoadSave() {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                assertEquals(lgame.getToken(i, j), game.getToken(i, j));
            }
        }
    ...
    public void testRewind() {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                assertEquals(g1.getToken(i, j), game.getToken(i, j));
            }
        }

        game.makeMove(2, 2, BlackPlayer); // c3
        Move m = game.popLastMove();
        assertEquals(m, new Move(2, 2, BlackPlayer));
	...
	* en cours de partie n'est pas forcément la meilleur.
        ...
        game.setGame(board, WhitePlayerPass, null);
